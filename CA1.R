...
# Convert categorical variables to human-readable labels
...
# Convert boolean features (1 and 2) to categorical 'Yes' and 'No'
...
# Replace coded missing values (97, 98, 99) with standard NA
data[data == 97 | data == 98 | data == 99] <- NA

# Check data structure to identify variable types (categorical, discrete, continuous)
str(data)

# Visualize distributions to further understand data type and check for missing values
...
# Visualize patterns of missing data, if applicable
missing_cols <- sum(colSums(is.na(data)) > 0)
if (missing_cols >= 2) {
  gg_miss_upset(data)
}

# Display count of missing values per variable
colSums(is.na(data))
